...
void NewtonPointSimulation<double>::evolve_with_gpu_2() {
    ...
    auto tensor_in_x_coordinates = mgr.tensorT<double>(x_coordinates_with_padding);
    auto tensor_in_y_coordinates = mgr.tensorT<double>(y_coordinates_with_padding);
    auto tensor_in_x_speeds = mgr.tensorT<double>(x_speeds_with_padding);
    auto tensor_in_y_speeds = mgr.tensorT<double>(y_speeds_with_padding);
    auto tensor_in_masses = mgr.tensorT<double>(masses_with_padding);

    const auto parameters_for_speed_shader =
        std::vector<std::shared_ptr<kp::Tensor>> { tensor_in_x_coordinates, tensor_in_y_coordinates, tensor_in_x_speeds,
                                                   tensor_in_y_speeds, tensor_in_masses };

    const auto path_to_speed_shader = std::filesystem::path("./shaders/newton_point_simulation/double/gpu_2.comp.spv");

    auto calculate_speeds = mgr.algorithm<uint32_t, double>(
        parameters_for_speed_shader, pasimulations::tools::readShader(path_to_speed_shader),
    ...
    const auto path_to_position_shader =
        std::filesystem::path("./shaders/newton_point_simulation/double/calculate_new_positions_2.comp.spv");

    auto calculate_new_positions = mgr.algorithm<uint32_t, double>(
        parameters_for_position_shader, pasimulations::tools::readShader(path_to_position_shader),
        kp::Workgroup { workgroups, 1, 1 }, {}, { timestep_ });

    mgr.sequence()
        ->record<kp::OpTensorSyncDevice>(parameters_for_speed_shader)
        ->record<kp::OpAlgoDispatch>(calculate_speeds, std::vector<double> { softening_radius_, G_, timestep_ })
        ->record<kp::OpAlgoDispatch>(calculate_new_positions, std::vector<double> { timestep_ })
...
template <>
void NewtonPointSimulation<double>::evolve_with_gpu_3() {
    ...
    const auto path_to_speed_shader = std::filesystem::path("./shaders/newton_point_simulation/double/gpu_3.comp.spv");
